...
"""
穴目予想：①残り「低」のとき、120通り穴ヒモTop3の2着候補から①を外す価値を固定条件で前方検証する。

固定条件
--------
- HIGH: AI本命=1C / CURRENT本命!=1C / イン補正後1着率<50%
- ①残り低: インAI3連対率 <60%（既存表示ルール固定）
...
IN_LOW_REMOVE
    ①残り低のレースだけ、イン艇がBASE 2着Top3に入っている場合に
    2着候補から除外する。3着候補はBASEのまま。
...
Usage:
python3 analysis/upset_in_low_second_outcome_prune_validate.py \
...
def build_row(record, boats, payout):
    ...
    # 既存①残り表示の「低」だけを事前条件として固定。
    in_trio_p = float(f["trio"].get(in_lane, 0.0))
    if in_trio_p >= 0.60:
        return None, "remain_not_low"
    ...
    in_in_second = in_lane in base_seconds
    in_cut = in_lane in cut
    remove_eligible = in_in_second and not in_cut

    prune_seconds = [lane for lane in base_seconds if lane != in_lane]
    # 念のため2着候補が空になる場合は変更しない。
    if remove_eligible and prune_seconds:
        prune_bets = b4.expand_bets(trio1, prune_seconds, base_thirds)
    else:
        prune_bets = set(base["bets"])
    ...
    return {
        ...
        "prune_bets": set(prune_bets),
        "in_in_second": in_in_second,
        "in_cut": in_cut,
...
def compare(rows):
    ...
    for r in rows:
        ...
        prune_hit = r["actual"] in r["prune_bets"]
        if prune_hit and not base_hit:
            gained += 1
        elif base_hit and not prune_hit:
            lost += 1
        elif base_hit and prune_hit:
            both += 1
...
def print_period(title, rows, skip):
    print("\n" + "=" * 122)
    print(f"【{title}】 HIGHかつ①残り低={len(rows)}R")
    print("=" * 122)
    if not rows:
        print("対象なし")
    ...
    in_second = sum(r["in_in_second"] for r in rows)
    cut = sum(r["in_cut"] for r in rows)
    eligible = sum(r["remove_eligible"] for r in rows)
    ...
    target_base_hit = sum(
        r["target_exact"] and r["actual"] in r["base_bets"]
        for r in rows
    )

    print(
        f"①の現状: OUTCOME 2着Top3内={in_second}/{len(rows)} ({pct(in_second,len(rows)):.2f}%) / "
        f"cut={cut}/{len(rows)} ({pct(cut,len(rows)):.2f}%) / "
        f"除外対象={eligible}/{len(rows)} ({pct(eligible,len(rows)):.2f}%)"
    )
    print(
    ...
    print(f"実TRIO1-①-* のうちBASE的中={target_base_hit}")

    base = bet_stats(rows, "base_bets")
    prune = bet_stats(rows, "prune_bets")
    ...
    print(
        f"IN_LOW_REMOVE     {prune['avg_points']:>7.2f}  {prune['hit_rate']:>7.2f}%"
        f"      {prune['roi100']:>8.2f}%        {prune['roi_fixed']:>8.2f}%"
    )
    print(
        f"差                {prune['avg_points']-base['avg_points']:+7.2f}  "
        f"{prune['hit_rate']-base['hit_rate']:+7.2f}pt"
        f"      {prune['roi100']-base['roi100']:+8.2f}pt       "
        f"{prune['roi_fixed']-base['roi_fixed']:+8.2f}pt"
    )
...
def main():
    if len(sys.argv) != 4:
        print(
            "Usage: python3 analysis/upset_in_low_second_outcome_prune_validate.py "
    ...
    print("①残り低と120通り穴ヒモTop3の2着除外を固定条件で検証中...", flush=True)
    ...
    print("=" * 122)
    print("穴目予想：①残り低 × 120通り穴ヒモTop3 2着除外 P1/P2/P3")
    print("=" * 122)
    print("固定: HIGH / ①残り低(インAI3連対<60%) / TRIO_TOP1 / 現行cut維持")
    print("除外: ①がP(2着|穴頭) Top3内のときだけ2着候補から外す")
    ...
    print("\n【判断ポイント】")
    print("1. ①残り低でも①がOUTCOME 2着Top3へ高頻度で入っているか")
    print("2. IN_LOW_REMOVEで的中率低下が小さく、ROIがP1/P2/P3で改善するか")
    print("3. 実TRIO1-①-*を複数期間で失うなら除外しない")
    print("4. 現行cutは変更しない")
