...
def load_venue_course_prior(conn, race_code, target_date, place_code):
    """対象レースより前の、今回場×実進入コース1着率を返す。

    STEP1と同じ考え方で、1着艇が一意かつ1着艇entry_courseが1～6の
    レースだけを採用する。採用レース1件につき各コースの分母は1。
    """
    ...
    with conn.cursor() as cur:
        cur.execute(
            sql,
            (place_code, target_date.isoformat(), target_date.isoformat(), race_code),
        )
...
def load_last_100(conn, player_id, target_date, target_race_code):
    ...
    for race_code, lane, rank, result_course, exhibition_course in rows:
        ...
        code = str(race_code)
        place = code[8:11] if len(code) >= 11 else "???"
        out.append({
            "race_code": code,
            "place": place,
            "course": course,
...
def player_counts(history, target_course, place_code):
    ...
    return {
        ...
        "pc_n": len(pc),
        "pc_w": sum(h["win"] for h in pc),
        "pvc_n": len(pvc),
        "pvc_w": sum(h["win"] for h in pvc),
        "sources": Counter(h["source"] for h in pc),
    }
...
def main():
    ...
    print("\n【6艇の基礎1着率】")
    print(
        "艇  選手ID   選手名             C   場×C(N/勝)      p0      選手×C(N/勝)  raw      平滑後p_pc   選手×場×C(N/勝) raw      基礎1着率"
    )
    print("-" * 174)
    ...
    for r in results:
        print(
            ...
            f"{r['course']}C  "
            f"{r['venue_n']:>6}/{r['venue_w']:<5} "
            f"{pct(r['p0'])}   "
            f"{r['pc_n']:>3}/{r['pc_w']:<3}     "
            f"{pct(r['pc_raw'])}   "
            f"{pct(r['p_pc'])}      "
            f"{r['pvc_n']:>3}/{r['pvc_w']:<3}          "
            f"{pct(r['pvc_raw'])}   "
            f"{pct(r['p_final'])}"
        )
    ...
    print("順位             : " + " > ".join(
        f"{r['lane']}号艇({r['p_final']*100:.2f}%)" for r in ordered
    ))
