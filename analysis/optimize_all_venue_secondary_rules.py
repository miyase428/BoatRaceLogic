...
過去3ブロックとの安定性を確認する。結果の ``params`` は設定生成器が
画面APIへ引き渡す。
"""
...
def primary_match(row: dict, place: str, course: int, variant: str, config: dict) -> bool:
    ...
    if course == 2 and variant == "makuri":
        return p["makuri"] >= threshold and row["st21"] == "上"
    if course == 3 or course == 5:
        return p["attack"] >= threshold
    if course == 4:
        return p["makuri"] >= threshold and row["st43"] == "上"
    if course == 6:
        return p["attack"] >= threshold and row["st65"] == "上"
    return False
...
def choose(rows: list[dict], key: str, end: date) -> dict:
    ...
    for params in GRIDS[key]:
        ...
        for _, st, en in blocks[1:]:
            ...
            if q["n"] < (20 if level == "double" else 10) or q["top3"] - b["top3"] < MIN_DELTA_TOP3:
                valid = False
        ...
        if hold["n"] < (20 if level == "double" else 10) or hold_delta < MIN_DELTA_TOP3:
            valid = False
        if valid:
            score = min([hold_delta, *train]) + 0.2 * (overall["top3"] - base["top3"])
            candidates.append((score, params, overall, hold, train, hold_delta))
    if not candidates:
        return {"enabled": False, "reason": "no_stable_candidate", "baseline": base}
    score, params, overall, hold, train, hold_delta = max(candidates, key=lambda x: (x[0], x[2]["n"]))
    return {"enabled": True, "params": list(params), "overall": overall, "baseline": base,
            "holdout": hold, "holdout_delta_top3": round(hold_delta, 2),
            "train_delta_top3": [round(x, 2) for x in train], "score": round(score, 2)}
