...
/**
 * 現行 FinalPredictionExporter のロジックを一切変更せず、
 * レース単位を複数プロセスで並列実行する高速版。
 *
 * 使い方:
 *   php analysis/export_final_prediction_fast.php 2026-08-01 2026-08-02
 *   php analysis/export_final_prediction_fast.php 2026-08-01 2026-08-02 4
 *
 * 第3引数: worker数（省略時4、最大8）
 *
 * 出力:
 *   analysis/output/final_prediction_boats_fast_YYYYMMDD_YYYYMMDD.csv
 *   analysis/output/final_prediction_races_fast_YYYYMMDD_YYYYMMDD.csv
 */
...
function usage(): never
{
    fwrite(STDERR,
        "使用方法:\n" .
        "  php analysis/export_final_prediction_fast.php START_DATE END_DATE [WORKERS]\n\n" .
        "例:\n" .
        "  php analysis/export_final_prediction_fast.php 2026-08-01 2026-08-02 4\n"
    );
    exit(1);
...
$boatsCsv = "{$outputDir}/final_prediction_boats_fast_{$startLabel}_{$endLabel}.csv";
$racesCsv = "{$outputDir}/final_prediction_races_fast_{$startLabel}_{$endLabel}.csv";
$tmpPrefix = "{$outputDir}/.tmp_prediction_fast_" . getmypid();

$startedAt = microtime(true);

echo "\n";
echo "============================================================\n";
echo "現行最終予想 並列高速CSV出力\n";
echo "============================================================\n";
echo "期間    : {$startDate} ～ {$endDate}\n";
echo "対象    : {$totalRaces}レース\n";
echo "workers : {$workers}\n";
echo "ロジック: FinalPredictionExporterをそのまま使用\n";
echo "============================================================\n\n";
...
if ($workers === 1) {
    $code = runWorker(1, $chunks[0], $tmpPrefix);
    if ($code !== 0) {
        exit($code);
    }
} else {
    foreach ($chunks as $idx => $chunk) {
        ...
        if ($pid === 0) {
            try {
                $code = runWorker($workerNo, $chunk, $tmpPrefix);
                exit($code);
            } catch (Throwable $e) {
