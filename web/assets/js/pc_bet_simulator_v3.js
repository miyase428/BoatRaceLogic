(function () {
    ...
    function expandTicketText(value, type) {
        ...
        source.split(' ').forEach(function (token) {
            ...
            const expected = type === 'exacta' ? 2 : 3;
            if (parts.length !== expected || parts.some(function (part) { return !/^[1-6]+$/.test(part); })) return;
    ...
    function setup() {
        waitForReady(function (tabs, recentPanel, recentButton, outcomes) {
            ...
            panel.innerHTML = ''
                ...
                + '    <div style="font-size:12px;color:#6b7785;margin-top:3px;">1回のフォーメーション選択から3連単と2連単を同時に候補表示。購入舟券は混在したまま払戻を合算します。</div>'
                ...
                + '      <label style="font-size:12px;font-weight:bold;color:#6b7785;">予算<input class="pc-sim-budget" type="number" min="100" step="100" value="1000" style="display:block;width:100%;box-sizing:border-box;margin-top:5px;"></label>'
                + '      <label style="font-size:12px;font-weight:bold;color:#6b7785;">自動配分<select class="pc-sim-mode" style="display:block;width:100%;box-sizing:border-box;margin-top:5px;"><option value="equal">均等配分</option><option value="minpayout">的中時最低払戻を重視</option></select></label>'
                + '      <div style="display:flex;gap:6px;justify-content:flex-end;"><button type="button" class="pc-sim-reload" style="width:auto;padding:8px 12px;background:#fff;color:#1683bd;border:1px solid #1683bd;">オッズ更新</button><button type="button" class="pc-sim-allocate" style="width:auto;padding:8px 12px;">自動再配分</button></div>'
                + '    </div>'
                + '    <div style="padding:10px;border:1px solid #d8cdbc;border-radius:6px;background:#f2ece2;">'
                + '      <div style="font-size:14px;font-weight:bold;color:#3f4b5a;margin-bottom:8px;">① フォーメーションを選択</div>'
                + '      <div class="pc-sim-formation" style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;"></div>'
                + '      <div style="display:flex;justify-content:flex-end;margin-top:8px;"><button type="button" class="pc-sim-selection-clear" style="width:auto;padding:6px 10px;background:#eee6da;color:#4b5866;border:1px solid #cbbda9;">選択クリア</button></div>'
                + '    </div>'
                + '    <div class="pc-sim-candidate-summary" style="display:grid;grid-template-columns:1fr 1fr;gap:8px;"></div>'
                + '    <div style="overflow-x:auto;max-height:390px;">'
                + '      <table class="pc-sim-candidate-table" style="width:100%;min-width:780px;border-collapse:collapse;">'
                ...
                + '      <button type="button" class="pc-sim-add-trifecta" style="width:auto;padding:6px 11px;background:#fff;color:#1683bd;border:1px solid #1683bd;">3連単を全部追加</button>'
                + '      <button type="button" class="pc-sim-add-exacta" style="width:auto;padding:6px 11px;background:#fff;color:#1683bd;border:1px solid #1683bd;">2連単を全部追加</button>'
                + '      <button type="button" class="pc-sim-add-all" style="width:auto;padding:6px 12px;">候補を全部追加</button>'
                + '    </div>'
                + '    <details style="border:1px solid #d8cdbc;border-radius:6px;background:#fffaf2;">'
                + '      <summary style="cursor:pointer;padding:8px 10px;font-size:12px;font-weight:bold;color:#4b5866;">直接入力・現在の絞り込みを追加（詳細）</summary>'
                + '      <div style="padding:0 10px 10px;display:grid;grid-template-columns:1fr 1fr;gap:8px;">'
                + '        <div><textarea class="pc-sim-exacta" rows="2" placeholder="2連単 例 1-2 / 1-24" style="width:100%;box-sizing:border-box;padding:7px;border:1px solid #cbbda9;border-radius:5px;background:#fffdf9;"></textarea><div style="display:flex;gap:5px;margin-top:5px;flex-wrap:wrap;"><button type="button" class="pc-sim-direct-exacta" style="width:auto;padding:5px 8px;">入力を追加</button><button type="button" class="pc-sim-import-exacta" style="width:auto;padding:5px 8px;background:#fff;color:#1683bd;border:1px solid #1683bd;">2連単表示中を追加</button></div></div>'
                + '        <div><textarea class="pc-sim-trifecta" rows="2" placeholder="3連単 例 1-2-4 / 1-234-234" style="width:100%;box-sizing:border-box;padding:7px;border:1px solid #cbbda9;border-radius:5px;background:#fffdf9;"></textarea><div style="display:flex;gap:5px;margin-top:5px;flex-wrap:wrap;"><button type="button" class="pc-sim-direct-trifecta" style="width:auto;padding:5px 8px;">入力を追加</button><button type="button" class="pc-sim-import-trifecta" style="width:auto;padding:5px 8px;background:#fff;color:#1683bd;border:1px solid #1683bd;">120通り表示中を追加</button></div></div>'
                + '      </div>'
                + '    </details>'
                + '    <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap;"><div style="font-size:14px;font-weight:bold;color:#3f4b5a;">② 購入する舟券</div><button type="button" class="pc-sim-clear" style="width:auto;padding:6px 12px;background:#eee6da;color:#4b5866;border:1px solid #cbbda9;">購入候補クリア</button></div>'
                ...
                + '    <div style="overflow-x:auto;"><table class="pc-sim-ticket-table" style="width:100%;min-width:820px;border-collapse:collapse;"><thead><tr style="background:#e8dfd2;"><th>種別</th><th>買い目</th><th>予測確率</th><th>オッズ</th><th>金額</th><th>単独払戻</th><th>削除</th></tr></thead><tbody></tbody></table></div>'
                + '    <div style="font-size:13px;font-weight:bold;color:#3f4b5a;margin-top:4px;">③ 結果別払戻</div>'
                + '    <div style="overflow-x:auto;max-height:480px;"><table class="pc-sim-outcome-table" style="width:100%;min-width:720px;border-collapse:collapse;"><thead><tr style="background:#e8dfd2;"><th>3連単結果</th><th>最終出目確率</th><th>的中券</th><th>払戻</th><th>収支</th></tr></thead><tbody></tbody></table></div>'
            ...
            const addTrifectaButton = panel.querySelector('.pc-sim-add-trifecta');
            const addExactaButton = panel.querySelector('.pc-sim-add-exacta');
            ...
            function formationKeys(type) {
                if (selected[0].size === 0 || selected[1].size === 0 || (type === 'trifecta' && selected[2].size === 0)) return [];
                const keys = [];
            ...
            function candidateLists() {
                const trifecta = formationKeys('trifecta').map(function (key) { return makeTicket('trifecta', key); });
                const exacta = formationKeys('exacta').map(function (key) { return makeTicket('exacta', key); });
                [trifecta, exacta].forEach(function (list) {
                    list.sort(function (a, b) {
                        if (a.probability === b.probability) return a.key.localeCompare(b.key, 'ja');
                        return b.probability - a.probability;
                    });
                });
                return {trifecta: trifecta, exacta: exacta};
            }
            ...
            function boatButtonStyle(button, boat, active) {
                const palette = {
                    1: ['#ffffff', '#202020', '#bfc3c7'], 2: ['#1f2937', '#ffffff', '#1f2937'],
                    3: ['#ef4444', '#ffffff', '#dc2626'], 4: ['#3b82f6', '#ffffff', '#2563eb'],
                    5: ['#facc15', '#222222', '#d4a900'], 6: ['#22c55e', '#ffffff', '#16a34a']
                };
            ...
            function renderFormation() {
                ...
                ['1着', '2着', '3着'].forEach(function (label, position) {
                    ...
                    title.textContent = label + (position === 2 ? '（3連単用）' : '');
                    ...
                    allButton.addEventListener('click', function () {
                        ...
                        if (!allActive) for (let boat = 1; boat <= 6; boat++) selected[position].add(boat);
                        renderFormation();
                        renderCandidates();
                    });
                    ...
                    for (let boat = 1; boat <= 6; boat++) {
                        ...
                        boatButtonStyle(btn, boat, selected[position].has(boat));
            ...
            function makeCombinationNode(key) {
                ...
                key.split('-').forEach(function (part, index) {
                    ...
                    const c = styles[boat];
                    badge.style.background = c[0]; badge.style.color = c[1]; badge.style.border = '1px solid ' + c[2];
            ...
            function candidateSummaryCard(label, list) {
                const div = document.createElement('div');
                div.style.cssText = 'padding:8px 10px;border:1px solid #d8cdbc;border-radius:6px;background:#fffdf9;font-size:12px;color:#4b5866;';
                const prob = list.reduce(function (sum, row) { return sum + row.probability; }, 0);
                const combo = combinedOdds(list);
                div.innerHTML = '<strong>' + label + '</strong>：' + list.length + '点 / 確率合計 <strong>' + pct(prob, 2) + '</strong> / 合成オッズ <strong>' + (combo === null ? '取得待ち' : combo.toFixed(2) + '倍') + '</strong>';
                return div;
            }

            function appendCandidateRows(list) {
                list.forEach(function (candidate) {
                    const tr = document.createElement('tr');
                    tr.style.borderTop = '1px solid #d8cdbc';
                    const type = document.createElement('td');
                    type.textContent = candidate.label;
                    type.style.cssText = 'padding:7px 8px;text-align:center;font-weight:bold;';
                    tr.appendChild(type);
                    ...
                    add.addEventListener('click', function () { addKeys(candidate.type, [candidate.key], true); });
            ...
            function renderCandidates() {
                ...
                candidateSummary.appendChild(candidateSummaryCard('3連単', lists.trifecta));
                candidateSummary.appendChild(candidateSummaryCard('2連単', lists.exacta));
                candidateBody.textContent = '';
                appendCandidateRows(lists.trifecta);
                appendCandidateRows(lists.exacta);
                if (!lists.trifecta.length && !lists.exacta.length) {
                    const tr = document.createElement('tr');
                    const td = document.createElement('td');
                    td.colSpan = 5;
                    td.textContent = '1着・2着を選ぶと2連単、さらに3着を選ぶと3連単も同時に候補表示されます。';
                    td.style.cssText = 'padding:14px;text-align:center;color:#6b7785;';
                    tr.appendChild(td);
                    candidateBody.appendChild(tr);
                }
                addTrifectaButton.disabled = lists.trifecta.length === 0;
                addExactaButton.disabled = lists.exacta.length === 0;
                addAllButton.disabled = lists.trifecta.length === 0 && lists.exacta.length === 0;
            }
            ...
            function coveredOutcomes(currentTickets) {
                return outcomes.filter(function (outcome) {
                    return currentTickets.some(function (ticket) { return ticket.stake > 0 && ticketHits(ticket, outcome); });
                });
            }
            ...
            function minCoveredPayout(currentTickets) {
                ...
                return Math.min.apply(null, covered.map(function (outcome) { return payoutFor(outcome, currentTickets); }));
            }

            function expectedPayout(currentTickets) {
                return outcomes.reduce(function (sum, outcome) { return sum + outcome.probability * payoutFor(outcome, currentTickets); }, 0);
            }
            ...
            function autoAllocate() {
                if (!tickets.length) { renderAll(); return; }
            ...
            function summaryCard(label, value, strong) {
                ...
                const l = document.createElement('div'); l.textContent = label; l.style.cssText = 'font-size:11px;color:#6b7785;';
                const v = document.createElement('div'); v.textContent = value; v.style.cssText = 'margin-top:3px;font-size:' + (strong ? '17px' : '15px') + ';font-weight:bold;color:' + (strong ? '#aa741f' : '#3f4b5a') + ';';
                div.appendChild(l); div.appendChild(v); return div;
            }
            ...
            function renderTickets() {
                ...
                if (!tickets.length) {
                    const tr = document.createElement('tr');
                    const td = document.createElement('td'); td.colSpan = 7; td.textContent = '上の候補から「追加」すると購入舟券がここに入ります。'; td.style.cssText = 'padding:14px;text-align:center;color:#6b7785;';
                    tr.appendChild(td); ticketBody.appendChild(tr); return;
                }
                tickets.forEach(function (ticket) {
                    const tr = document.createElement('tr'); tr.style.borderTop = '1px solid #d8cdbc';
                    const type = document.createElement('td'); type.textContent = ticket.label; type.style.cssText = 'padding:7px 8px;text-align:center;'; tr.appendChild(type);
                    const combo = document.createElement('td'); combo.style.cssText = 'padding:7px 8px;'; combo.appendChild(makeCombinationNode(ticket.key)); tr.appendChild(combo);
                    [pct(ticket.probability, 2), ticket.odds ? ticket.odds.toFixed(1) : '-'].forEach(function (value) {
                        const td = document.createElement('td'); td.textContent = value; td.style.cssText = 'padding:7px 8px;text-align:right;'; tr.appendChild(td);
                    });
                    const stakeCell = document.createElement('td'); stakeCell.style.cssText = 'padding:5px 8px;text-align:right;';
                    const input = document.createElement('input'); input.type = 'number'; input.min = '0'; input.step = '100'; input.value = String(ticket.stake); input.style.cssText = 'width:92px;box-sizing:border-box;text-align:right;padding:5px 7px;';
                    input.addEventListener('change', function () {
                        ticket.stake = Math.max(0, Math.floor(number(input.value) / UNIT) * UNIT); input.value = String(ticket.stake); renderSummary(); renderOutcomes();
                        const payoutCell = tr.querySelector('.pc-sim-single-payout'); if (payoutCell) payoutCell.textContent = ticket.odds && ticket.stake > 0 ? yen(ticket.stake * ticket.odds) : '-';
                    });
                    stakeCell.appendChild(input); tr.appendChild(stakeCell);
                    const payout = document.createElement('td'); payout.className = 'pc-sim-single-payout'; payout.textContent = ticket.odds && ticket.stake > 0 ? yen(ticket.stake * ticket.odds) : '-'; payout.style.cssText = 'padding:7px 8px;text-align:right;font-weight:bold;color:#7b6332;'; tr.appendChild(payout);
                    const removeCell = document.createElement('td'); removeCell.style.cssText = 'padding:5px 8px;text-align:center;';
                    const remove = document.createElement('button'); remove.type = 'button'; remove.textContent = '削除'; remove.style.cssText = 'width:auto;padding:5px 9px;background:#eee6da;color:#4b5866;border:1px solid #cbbda9;';
                    remove.addEventListener('click', function () {
                        tickets = tickets.filter(function (row) { return row.id !== ticket.id; });
                        if (tickets.length) autoAllocate(); else { showMessage('', false); renderAll(); }
                        renderCandidates();
                    });
                    removeCell.appendChild(remove); tr.appendChild(removeCell); ticketBody.appendChild(tr);
                });
            }
            ...
            function renderOutcomes() {
                ...
                covered.forEach(function (row) {
                    ...
                    const values = [row.outcome.key, pct(row.outcome.probability, 3), row.hitTickets.map(function (ticket) { return ticket.label + ' ' + ticket.key; }).join(' ＋ '), yen(row.payout), (net >= 0 ? '+' : '') + yen(net)];
                    values.forEach(function (value, index) {
                        const td = document.createElement('td'); td.textContent = value; td.style.padding = '7px 8px'; td.style.textAlign = index === 0 || index === 2 ? 'left' : 'right';
                        if (index === 0 || index === 3) td.style.fontWeight = 'bold'; if (index === 4) td.style.color = net >= 0 ? '#2f789f' : '#a74932'; tr.appendChild(td);
                    });
            ...
            function activateSimulator() {
                document.querySelectorAll('.pc-main-tab').forEach(function (tab) { tab.classList.toggle('is-active', tab.dataset.pcMainTab === 'simulator'); });
                document.querySelectorAll('.pc-main-tab-panel').forEach(function (tabPanel) {
                    const active = tabPanel.dataset.pcMainPanel === 'simulator'; tabPanel.classList.toggle('is-active', active); tabPanel.hidden = !active;
                });
                try { sessionStorage.setItem(STORAGE_KEY, 'simulator'); } catch (e) {}
            }

            tabButton.addEventListener('click', activateSimulator);
            selectionClearButton.addEventListener('click', function () { selected.forEach(function (set) { set.clear(); }); renderFormation(); renderCandidates(); });
            addTrifectaButton.addEventListener('click', function () {
                const keys = formationKeys('trifecta'); const added = addKeys('trifecta', keys, true); showMessage(added ? '3連単 ' + added + '点を追加しました。' : '3連単はすべて追加済みです。', false);
            });
            addExactaButton.addEventListener('click', function () {
                const keys = formationKeys('exacta'); const added = addKeys('exacta', keys, true); showMessage(added ? '2連単 ' + added + '点を追加しました。' : '2連単はすべて追加済みです。', false);
            });
            addAllButton.addEventListener('click', function () {
                const tri = addKeys('trifecta', formationKeys('trifecta'), false); const ex = addKeys('exacta', formationKeys('exacta'), false);
                if (tri + ex > 0) autoAllocate(); else renderAll();
                renderCandidates(); showMessage(tri + ex > 0 ? '3連単 ' + tri + '点 / 2連単 ' + ex + '点を追加しました。' : 'すべて追加済みです。', false);
            });
            directExactaButton.addEventListener('click', function () {
                const keys = expandTicketText(exactaInput.value, 'exacta'); const added = addKeys('exacta', keys, true); showMessage(added ? '2連単 ' + added + '点を追加しました。' : '追加できる2連単がありません。', added === 0);
            });
            directTrifectaButton.addEventListener('click', function () {
                const keys = expandTicketText(trifectaInput.value, 'trifecta'); const added = addKeys('trifecta', keys, true); showMessage(added ? '3連単 ' + added + '点を追加しました。' : '追加できる3連単がありません。', added === 0);
            });
            importExactaButton.addEventListener('click', function () {
                const keys = visibleExactaKeys(); const added = addKeys('exacta', keys, true); showMessage('2連単の表示中から ' + added + '点追加しました。', false);
            });
            importTrifectaButton.addEventListener('click', function () {
                const keys = visibleTrifectaKeys(); const added = addKeys('trifecta', keys, true); showMessage('3連単の表示中から ' + added + '点追加しました。', false);
            });
            clearButton.addEventListener('click', function () { tickets = []; showMessage('', false); renderAll(); renderCandidates(); });
            ...
            renderFormation(); renderCandidates(); renderAll(); loadOdds(false);
